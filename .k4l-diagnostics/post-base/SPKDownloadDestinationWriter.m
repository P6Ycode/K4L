...
@interface SPKDownloadDestinationWriter ()
...
+ (SPKGalleryFile *)saveFileURLToGallery:(NSURL *)fileURL
                                metadata:(SPKGallerySaveMetadata *)metadata
                                   error:(NSError **)error {
    return [self saveFileURLToGallery:fileURL
                             metadata:metadata
                            mediaKind:SPKDownloadMediaKindUnknown
                                error:error];
}
...
+ (SPKGalleryFile *)saveFileURLToGallery:(NSURL *)fileURL
    ...
    case SPKDownloadMediaKindVideo:
        galleryType = SPKGalleryMediaTypeVideo;
        break;
    case SPKDownloadMediaKindAudio:
        galleryType = SPKGalleryMediaTypeAudio;
        break;
    case SPKDownloadMediaKindImage:
        galleryType = SPKGalleryMediaTypeImage;
        break;
    default:
        galleryType = [self isAudioFileAtURL:fileURL]
                          ? SPKGalleryMediaTypeAudio
                          : ([self isVideoFileAtURL:fileURL]
        ...
        if (completion) {
            completion(nil, nil,
                       SPKDownloadError(SPKDownloadErrorAudioPhotosUnsupported,
                                        @"Standalone audio downloads are disabled in K4L.",
                                        @"Download the original photo or video instead."));
        }
        return;
    }
    BOOL isDirectory = NO;
...
- (void)saveToGallery:(NSURL *)fileURL
    ...
    SPKGalleryFile *file = [[self class] saveFileURLToGallery:fileURL
                                                     metadata:metadata
                                                    mediaKind:itemRequest.mediaKind
                                                        error:&error];
    if (!file) {
        if (completion)
            completion(nil, nil,
