...
static NSString *SCIStringFromEntitlementValue(id value) {
...
static NSString *SCIAccessGroupFromEntitlements(void) {
	...
	NSString *accessGroup = SCIStringFromEntitlementValue(entitlements[@"keychain-access-groups"]);
	if (accessGroup.length > 0) {
		return accessGroup;
	}

	NSString *applicationIdentifier = SCIStringFromEntitlementValue(entitlements[@"application-identifier"]);
	return applicationIdentifier.length > 0 ? applicationIdentifier : nil;
}

static NSString *SCIAccessGroupFromSentinelKeychainItem(void) {
	...
	NSDictionary *query = @{
		...
		(__bridge NSString *)kSecAttrService: @"SCISideloadFix",
	...
	if (status != errSecSuccess || !result) {
		...
		SCISideloadLog(@"Unable to resolve keychain access group from sentinel item; status=%d", (int)status);
		return nil;
	}
...
static NSString *SCISideloadAccessGroup(void) {
	static NSString *accessGroup;
	static dispatch_once_t onceToken;
	dispatch_once(&onceToken, ^{
		NSString *entitlementGroup = SCIAccessGroupFromEntitlements();
		BOOL resolvedFromEntitlements = entitlementGroup.length > 0;
		accessGroup = [entitlementGroup copy];
		if (accessGroup.length == 0) {
			accessGroup = [SCIAccessGroupFromSentinelKeychainItem() copy];
		}

		if (accessGroup.length > 0) {
			SCISideloadLog(@"Resolved keychain access group from %@", resolvedFromEntitlements ? @"entitlements" : @"sentinel item");
		} else {
			SCISideloadLog(@"No keychain access group resolved; keychain calls will pass through unchanged");
		}
	});
	return accessGroup;
}

static CFDictionaryRef SCICopyDictionaryByInjectingAccessGroup(CFDictionaryRef dictionary, NSString **mode) {
	...
	NSString *accessGroup = SCISideloadAccessGroup();
	if (accessGroup.length == 0) {
		if (mode) *mode = @"pass-through";
		return NULL;
	}
...
static void SCILogSecOperation(NSString *operation, OSStatus status, CFAbsoluteTime startedAt, NSString *mode) {
	...
	SCISideloadLog(@"Keychain %@ status=%d duration=%.2fms mainThread=%@ accessGroup=%@",
		operation,
		(int)status,
...
static OSStatus zxSecItemAdd(CFDictionaryRef attributes, CFTypeRef *result) {
	...
	NSString *mode = nil;
	CFDictionaryRef patchedAttributes = SCICopyDictionaryByInjectingAccessGroup(attributes, &mode);
	OSStatus status = origSecItemAdd(patchedAttributes ?: attributes, result);
	if (patchedAttributes) CFRelease(patchedAttributes);
	SCILogSecOperation(@"SecItemAdd", status, startedAt, mode);
	return status;
}
...
static OSStatus zxSecItemCopyMatching(CFDictionaryRef query, CFTypeRef *result) {
	CFAbsoluteTime startedAt = CFAbsoluteTimeGetCurrent();
	NSString *mode = nil;
	CFDictionaryRef patchedQuery = SCICopyDictionaryByInjectingAccessGroup(query, &mode);
	OSStatus status = origSecItemCopyMatching(patchedQuery ?: query, result);
	if (patchedQuery) CFRelease(patchedQuery);
	SCILogSecOperation(@"SecItemCopyMatching", status, startedAt, mode);
	return status;
}
...
static OSStatus zxSecItemUpdate(CFDictionaryRef query, CFDictionaryRef attributesToUpdate) {
	...
	if (patchedQuery) CFRelease(patchedQuery);
	SCILogSecOperation(@"SecItemUpdate", status, startedAt, mode);
	return status;
}
...
static OSStatus zxSecItemDelete(CFDictionaryRef query) {
	...
	if (patchedQuery) CFRelease(patchedQuery);
	SCILogSecOperation(@"SecItemDelete", status, startedAt, mode);
	return status;
}
...
void rebindSecFuncs() {
	...
	SCISideloadLog(@"rebind_symbols result=%d", rebind_symbols(rebinds, 4));
}
