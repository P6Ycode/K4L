...
static NSString * const kSPKSectionIconCellIdentifier = @"SPKSectionIconCell";
static NSString * const kSPKSectionIconHeaderIdentifier = @"SPKSectionIconHeader";
@interface SPKSectionIconPickerItem : NSObject
@property (nonatomic, copy) NSString *iconName;
@property (nonatomic, copy) NSString *title;
@property (nonatomic, copy) NSString *searchText;
@end

@implementation SPKSectionIconPickerItem
@end

@interface SPKSectionIconPickerCell : UICollectionViewCell
@property (nonatomic, strong) UIImageView *iconView;
@property (nonatomic, strong) UILabel *titleLabel;
@property (nonatomic, strong) UIImageView *checkmarkView;
- (void)configureWithItem:(SPKSectionIconPickerItem *)item image:(UIImage *)image selected:(BOOL)selected;
@end

static NSString *SPKSectionIconPickerCellTitle(NSString *title) {
...
@implementation SPKSectionIconPickerCell
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _iconView.tintColor = [SPKUtils SPKColor_InstagramPrimaryText];
    [self.contentView addSubview:_iconView];

    _titleLabel = [[UILabel alloc] initWithFrame:CGRectZero];
    _titleLabel.translatesAutoresizingMaskIntoConstraints = NO;
    _titleLabel.font = [UIFont systemFontOfSize:10.0 weight:UIFontWeightRegular];
    _titleLabel.textColor = [SPKUtils SPKColor_InstagramSecondaryText];
...
- (void)configureWithItem:(SPKSectionIconPickerItem *)item image:(UIImage *)image selected:(BOOL)selected {
    self.titleLabel.text = SPKSectionIconPickerCellTitle(item.title);
...
@interface SPKSectionIconPickerHeaderView : UICollectionReusableView
@property (nonatomic, strong) UILabel *titleLabel;
- (void)configureWithTitle:(NSString *)title;
@end

@implementation SPKSectionIconPickerHeaderView
...
@interface SPKActionSectionIconPickerViewController () <UICollectionViewDataSource, UICollectionViewDelegate, UISearchResultsUpdating>

@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) NSArray<NSArray<SPKSectionIconPickerItem *> *> *allSections;
@property (nonatomic, strong) NSArray<NSArray<SPKSectionIconPickerItem *> *> *filteredSections;
...
- (void)viewDidLoad {
    ...
    [self.collectionView registerClass:[SPKSectionIconPickerCell class] forCellWithReuseIdentifier:kSPKSectionIconCellIdentifier];
    [self.collectionView registerClass:[SPKSectionIconPickerHeaderView class]
            forSupplementaryViewOfKind:UICollectionElementKindSectionHeader
                   withReuseIdentifier:kSPKSectionIconHeaderIdentifier];
...
- (SPKSectionIconPickerItem *)itemWithIconName:(NSString *)iconName title:(NSString *)title {
    SPKSectionIconPickerItem *item = [[SPKSectionIconPickerItem alloc] init];
    item.iconName = iconName ?: @"more";
    item.title = title.length > 0 ? title : [SPKInstagramIconCatalog displayNameForIconName:item.iconName];
    item.searchText = [[NSString stringWithFormat:@"%@ %@",
                        [SPKInstagramIconCatalog searchTextForIconName:item.iconName],
                        [item.title lowercaseString]] lowercaseString];
    return item;
}

- (void)buildSections {
    NSMutableArray<SPKSectionIconPickerItem *> *shortcutItems = [NSMutableArray array];
    NSMutableArray<SPKSectionIconPickerItem *> *instagramItems = [NSMutableArray array];
    NSMutableSet<NSString *> *seenIconNames = [NSMutableSet set];

    void (^addShortcutItem)(SPKSectionIconPickerItem *) = ^(SPKSectionIconPickerItem *item) {
    ...
    void (^addInstagramItem)(SPKSectionIconPickerItem *) = ^(SPKSectionIconPickerItem *item) {
        if (item.iconName.length == 0 || [seenIconNames containsObject:item.iconName]) {
            return;
        }
        [seenIconNames addObject:item.iconName];
    ...
    if (self.selectedIconName.length > 0 &&
        ...
        ![[SPKInstagramIconCatalog availableInstagramIconNames] containsObject:self.selectedIconName]) {
        addShortcutItem([self itemWithIconName:self.selectedIconName title:[NSString stringWithFormat:@"Current: %@", [SPKInstagramIconCatalog displayNameForIconName:self.selectedIconName]]]);
    }
    ...
    for (NSString *iconName in [SPKInstagramIconCatalog availableInstagramIconNames]) {
        addInstagramItem([self itemWithIconName:iconName title:[SPKInstagramIconCatalog displayNameForIconName:iconName]]);
    }

    NSMutableArray<NSArray<SPKSectionIconPickerItem *> *> *sections = [NSMutableArray array];
...
- (UIImage *)imageForIconName:(NSString *)iconName {
    ...
    UIImage *image = [SPKAssetUtils instagramIconNamed:iconName
                                             pointSize:28.0
                                                source:[SPKInstagramIconCatalog isInstagramBundleIconName:iconName] ? SPKAssetCatalogSourceFBSharedFramework : SPKAssetCatalogSourceAutomatic
...
- (void)scrollToSelectedIconIfNeeded {
    for (NSUInteger section = 0; section < self.filteredSections.count; section++) {
        NSArray<SPKSectionIconPickerItem *> *items = self.filteredSections[section];
        NSUInteger index = [items indexOfObjectPassingTest:^BOOL(SPKSectionIconPickerItem *item, NSUInteger idx, BOOL *stop) {
            (void)idx;
            (void)stop;
...
- (void)filterForSearchText:(NSString *)searchText {
    ...
    NSMutableArray<NSArray<SPKSectionIconPickerItem *> *> *filteredSections = [NSMutableArray array];
    ...
    for (NSUInteger section = 0; section < self.allSections.count; section++) {
        NSMutableArray<SPKSectionIconPickerItem *> *matches = [NSMutableArray array];
        for (SPKSectionIconPickerItem *item in self.allSections[section]) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    SPKSectionIconPickerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:kSPKSectionIconCellIdentifier forIndexPath:indexPath];
    SPKSectionIconPickerItem *item = self.filteredSections[indexPath.section][indexPath.item];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView
    ...
    SPKSectionIconPickerHeaderView *header = [collectionView dequeueReusableSupplementaryViewOfKind:kind
                                                                                withReuseIdentifier:kSPKSectionIconHeaderIdentifier
