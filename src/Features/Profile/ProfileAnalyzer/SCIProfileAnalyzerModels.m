...
+ (instancetype)userFromAPIDict:(NSDictionary *)d {
    ...
    NSString *pk = sciStringFromValue(d[@"pk"] ?: d[@"pk_id"] ?: d[@"id"]);
    ...
    u.profilePicID = sciStringFromValue(d[@"profile_pic_id"]);
...
+ (instancetype)userFromIGUserObject:(id)igUser {
    ...
    NSString *pk = sciStringFromValue(sciFieldCacheValue(igUser, @"strong_id__")
                                      ?: sciFieldCacheValue(igUser, @"pk")
                                      ?: sciFieldCacheValue(igUser, @"pk_id"));
    if (!pk.length) return nil;

    SCIProfileAnalyzerUser *u = [self new];
    u.pk = pk;
    id un = sciFieldCacheValue(igUser, @"username");
    u.username = [un isKindOfClass:[NSString class]] ? un : @"";
    id fn = sciFieldCacheValue(igUser, @"full_name");
    if ([fn isKindOfClass:[NSString class]]) u.fullName = fn;
    id pic = sciFieldCacheValue(igUser, @"profile_pic_url");
    if ([pic isKindOfClass:[NSString class]]) u.profilePicURL = pic;
    u.profilePicID = sciStringFromValue(sciFieldCacheValue(igUser, @"profile_pic_id"));
    u.isPrivate = [sciFieldCacheValue(igUser, @"is_private") boolValue];
    u.isVerified = [sciFieldCacheValue(igUser, @"is_verified") boolValue];
    return u;
}
