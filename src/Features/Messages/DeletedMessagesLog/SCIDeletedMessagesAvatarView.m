...
@interface SCIDeletedMessagesAvatarView ()
...
@implementation SCIDeletedMessagesAvatarView

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        self.clipsToBounds = YES;
        self.backgroundColor = [SCIUtils SCIColor_InstagramTertiaryBackground];
        ...
        _placeholderView.tintColor = [SCIUtils SCIColor_InstagramSecondaryText];
        _placeholderView.image = [SCIAssetUtils instagramIconNamed:@"user_circle" pointSize:44.0 renderingMode:UIImageRenderingModeAlwaysTemplate];
...
- (void)prepareForReuse {
    ...
    self.placeholderView.image = [SCIAssetUtils instagramIconNamed:@"user_circle" pointSize:44.0 renderingMode:UIImageRenderingModeAlwaysTemplate];
}
...
- (void)configureAsGroupWithThreadId:(NSString *)threadId photoURL:(NSString *)photoURL {
    ...
    self.placeholderView.image = glyph ?: [SCIAssetUtils instagramIconNamed:@"user_circle" pointSize:24.0 renderingMode:UIImageRenderingModeAlwaysTemplate];
    ...
    UIImage *warm = [[SCIDeletedMessagesAvatarCache shared] cachedImageForPK:cacheKey];
    if (warm) { [self applyImage:warm]; return; }

    __weak typeof(self) weakSelf = self;
    NSString *requested = cacheKey;
    [[SCIDeletedMessagesAvatarCache shared] avatarForPK:cacheKey urlString:photoURL completion:^(UIImage *image) {
...
- (void)configureWithPK:(NSString *)pk urlString:(NSString *)urlString {
    ...
    UIImage *warm = [[SCIDeletedMessagesAvatarCache shared] cachedImageForPK:pk];
    if (warm) {
    ...
    [[SCIDeletedMessagesAvatarCache shared] avatarForPK:pk urlString:urlString completion:^(UIImage *image) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf || !image) return;
