...
@implementation SCIBulkSelectionDestination
+ (instancetype)destinationWithIdentifier:(NSString *)identifier
                                    title:(NSString *)title
                                 iconName:(NSString *)iconName {
    SCIBulkSelectionDestination *dest = [self new];
...
@interface SCIBulkSelectionCell : UICollectionViewCell
...
@implementation SCIBulkSelectionCell

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        self.contentView.backgroundColor = [SCIUtils SCIColor_InstagramTertiaryBackground];
        ...
        _videoBadge = [[UIImageView alloc] initWithImage:[SCIAssetUtils instagramIconNamed:@"video_filled" pointSize:14.0]];
...
@interface SCIBulkMediaSelectionViewController () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
@property (nonatomic, copy) NSArray<SCIBulkSelectionItem *> *items;
@property (nonatomic, copy) NSArray<SCIBulkSelectionDestination *> *destinations;
@property (nonatomic, copy) SCIBulkSelectionCompletion completion;
...
static const void *kSCIBulkSelectionDestinationIdentifierKey = &kSCIBulkSelectionDestinationIdentifierKey;

static NSCache<NSURL *, UIImage *> *SCIBulkSelectionThumbnailCache(void) {
...
@implementation SCIBulkMediaSelectionViewController

- (instancetype)initWithItems:(NSArray<SCIBulkSelectionItem *> *)items
                 destinations:(NSArray<SCIBulkSelectionDestination *> *)destinations
                   completion:(SCIBulkSelectionCompletion)completion {
...
+ (void)presentFromViewController:(UIViewController *)presenter
                             items:(NSArray<SCIBulkSelectionItem *> *)items
                      destinations:(NSArray<SCIBulkSelectionDestination *> *)destinations
                        completion:(SCIBulkSelectionCompletion)completion {
    ...
    SCIBulkMediaSelectionViewController *vc =
        [[SCIBulkMediaSelectionViewController alloc] initWithItems:items
...
- (void)viewDidLoad {
    ...
    [self.collectionView registerClass:[SCIBulkSelectionCell class] forCellWithReuseIdentifier:@"cell"];
    [self.view addSubview:self.collectionView];

    SCIMediaChromeSetLeadingTopBarItems(self.navigationItem,
                                        @[ SCIMediaChromeTopBarButtonItem(@"xmark", self, @selector(cancel)) ]);

    self.selectAllItem = SCIMediaChromeTopBarButtonItem(@"circle", self, @selector(toggleSelectAll));
    self.selectAllItem.accessibilityLabel = @"Select all";
    SCIMediaChromeSetTrailingTopBarItems(self.navigationItem, @[ self.selectAllItem ]);
    ...
    for (SCIBulkSelectionDestination *dest in self.destinations) {
        UIBarButtonItem *item = SCIMediaChromeBottomBarButtonItem(dest.iconName ?: @"download",
                                                                  dest.title,
                                                                  self,
                                                                  @selector(toolbarItemTapped:));
        objc_setAssociatedObject(item, kSCIBulkSelectionDestinationIdentifierKey, dest.identifier, OBJC_ASSOCIATION_COPY_NONATOMIC);
        [destinationItems addObject:item];
    }
    self.destinationBarItems = destinationItems;
    self.toolbarItems = SCIMediaChromeBottomToolbarItems(destinationItems);
...
- (void)updateChrome {
    ...
    self.selectAllItem.image = SCIMediaChromeTopBarIcon(resource);
...
- (void)toolbarItemTapped:(UIBarButtonItem *)sender {
    NSString *destIdentifier = objc_getAssociatedObject(sender, kSCIBulkSelectionDestinationIdentifierKey);
    if (destIdentifier) {
...
- (void)confirmWithDestination:(NSString *)destinationIdentifier {
    ...
    SCIBulkSelectionCompletion completion = self.completion;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    SCIBulkSelectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cell" forIndexPath:indexPath];
    SCIBulkSelectionItem *item = self.items[(NSUInteger)indexPath.item];
...
- (void)loadThumbnailForCell:(SCIBulkSelectionCell *)cell item:(SCIBulkSelectionItem *)item {
    ...
    UIImage *cached = [SCIBulkSelectionThumbnailCache() objectForKey:url];
    if (cached) {
    ...
    void (^apply)(UIImage *) = ^(UIImage *image) {
        if (!image) return;
        [SCIBulkSelectionThumbnailCache() setObject:image forKey:url];
        dispatch_async(dispatch_get_main_queue(), ^{
            SCIBulkSelectionCell *strongCell = weakCell;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SCIBulkSelectionCell *cell = (SCIBulkSelectionCell *)[collectionView cellForItemAtIndexPath:indexPath];
    if ([cell isKindOfClass:[SCIBulkSelectionCell class]]) {
