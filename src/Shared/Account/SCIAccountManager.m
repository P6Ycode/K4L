...
NSNotificationName const SCIAccountDidChangeNotification = @"SCIAccountDidChangeNotification";

static NSString * const kSCIAccountRosterDefaultsKey = @"sci_account_roster";
...
+ (instancetype)shared {
    static SCIAccountManager *shared;
...
static NSString *SCIAccountUsernameFromSession(id session) {
    @try {
...
static BOOL SCIStringsEqual(NSString *a, NSString *b) {
    if (a == b) return YES;
    if (!a || !b) return NO;
...
- (void)refreshCurrentAccount {
    ...
    NSString *username = pk.length > 0 ? SCIAccountUsernameFromSession(session) : nil;
...
+ (NSString *)currentAccountPK {
    SCIAccountManager *manager = [self shared];
...
+ (void)recordAccountPK:(NSString *)pk username:(NSString *)username {
    ...
    NSDictionary *stored = [defaults dictionaryForKey:kSCIAccountRosterDefaultsKey];
    ...
    roster[pk] = entry;
    [defaults setObject:roster forKey:kSCIAccountRosterDefaultsKey];
}

+ (NSArray<NSDictionary *> *)knownAccounts {
    NSDictionary *stored = [[NSUserDefaults standardUserDefaults] dictionaryForKey:kSCIAccountRosterDefaultsKey];
