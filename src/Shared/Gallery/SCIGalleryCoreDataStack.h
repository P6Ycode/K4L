...
typedef NS_ENUM(NSInteger, SCIGalleryImportConflictStrategy) {
    SCIGalleryImportConflictStrategySkip = 0,       ///< Leave the existing file untouched.
    SCIGalleryImportConflictStrategyClaim = 1,      ///< Re-assign it to the importing account (moves it).
    SCIGalleryImportConflictStrategyDuplicate = 2,  ///< Add a separate copy owned by the importing account.
};
...
/// Non-destructive import: inserts every SCIGalleryFile from an exported bundle's
