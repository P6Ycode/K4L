...
@interface SCITrimDoneOption : NSObject
@property (nonatomic, copy) NSString *title;
/// Reported back on `SCITrimResult.destinationTag` when chosen.
...
/// What we're editing. Defaults to `SCITrimMediaKindVideo`.
@property (nonatomic, assign) SCITrimMediaKind mediaKind;

/// When YES, the editor exposes the "Single frame" mode that exports a still
/// image instead of a clip. Only meaningful for video. Defaults to YES.
@property (nonatomic, assign) BOOL allowsSingleFrame;
...
/// the chosen one is reported on `SCITrimResult.destinationTag`. When empty,
/// Done is a plain confirm button (gallery flow handles routing afterwards).
@property (nonatomic, copy, nullable) NSArray<SCITrimDoneOption *> *doneOptions;
