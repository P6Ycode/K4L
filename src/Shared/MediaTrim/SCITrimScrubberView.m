...
static CGFloat const kSCITrimHandleWidth = 18.0;
static CGFloat const kSCITrimBorderThickness = 3.0;
static CGFloat const kSCITrimGrabThreshold = 36.0;
static NSInteger const kSCITrimThumbnailCount = 16;
static CGFloat const kSCITrimCornerRadius = 8.0;

typedef NS_ENUM(NSInteger, SCITrimDragTarget) {
    SCITrimDragTargetNone = 0,
    SCITrimDragTargetLeftHandle,
    SCITrimDragTargetRightHandle,
    SCITrimDragTargetPlayhead,
};

@interface SCITrimScrubberView ()
...
@property (nonatomic, assign) SCITrimDragTarget dragTarget;
@property (nonatomic, strong) AVAssetImageGenerator *imageGenerator;
@end

@implementation SCITrimScrubberView
...
- (void)setupViews {
    ...
    _filmstripContainer.layer.cornerRadius = kSCITrimCornerRadius;
...
- (UIView *)makeHandleWithAccent:(UIColor *)accent {
    ...
    handle.layer.cornerRadius = kSCITrimCornerRadius;
...
- (void)loadThumbnailsForAsset:(AVAsset *)asset {
    ...
    for (NSInteger i = 0; i < kSCITrimThumbnailCount; i++) {
        NSTimeInterval t = (total * (i + 0.5)) / kSCITrimThumbnailCount;
    ...
    [generator generateCGImagesAsynchronouslyForTimes:times
        ...
        NSInteger slot = (NSInteger)floor((requested * kSCITrimThumbnailCount) / total);
        slot = MAX(0, MIN(slot, kSCITrimThumbnailCount - 1));
...
- (void)layoutSelection {
    ...
    _selectionTopBorder.frame = CGRectMake(startX, 0, MAX(0, endX - startX), kSCITrimBorderThickness);
    _selectionBottomBorder.frame = CGRectMake(startX, h - kSCITrimBorderThickness, MAX(0, endX - startX), kSCITrimBorderThickness);

    CGFloat leftX = MAX(0, startX - kSCITrimHandleWidth);
    _leftHandle.frame = CGRectMake(leftX, 0, kSCITrimHandleWidth, h);
    CGFloat rightX = MIN(self.bounds.size.width - kSCITrimHandleWidth, endX);
    _rightHandle.frame = CGRectMake(rightX, 0, kSCITrimHandleWidth, h);
...
- (void)handlePan:(UIPanGestureRecognizer *)pan {
    ...
    if (pan.state == UIGestureRecognizerStateEnded ||
        ...
        _dragTarget = SCITrimDragTargetNone;
...
- (SCITrimDragTarget)dragTargetForPoint:(CGPoint)p {
    if (_singleFrameMode) {
        return SCITrimDragTargetPlayhead;
    }
    CGFloat startX = [self xForTime:_startTime];
    CGFloat endX = [self xForTime:_endTime];
    ...
    if (distLeft <= kSCITrimGrabThreshold || distRight <= kSCITrimGrabThreshold) {
        return (distLeft <= distRight) ? SCITrimDragTargetLeftHandle : SCITrimDragTargetRightHandle;
    }
    if (p.x > startX && p.x < endX) {
        return SCITrimDragTargetPlayhead;
    }
    // Outside selection: grab the nearer handle so it extends toward the touch.
    return (p.x <= startX) ? SCITrimDragTargetLeftHandle : SCITrimDragTargetRightHandle;
}

- (void)applyDragAtPoint:(CGPoint)p {
    NSTimeInterval t = [self timeForX:p.x];
    switch (_dragTarget) {
        case SCITrimDragTargetLeftHandle: {
        ...
        case SCITrimDragTargetRightHandle: {
        ...
        case SCITrimDragTargetPlayhead: {
            if (!_singleFrameMode) {
                t = MAX(_startTime, MIN(t, _endTime));
            }
            self.playheadTime = t;
