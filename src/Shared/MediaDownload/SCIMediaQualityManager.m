...
SCIMediaNormalizedAndSortedVariants(NSArray<NSDictionary *> *variants) {
...
static NSString *SCIMediaEstimatedSizeString(NSInteger bandwidth,
...
static NSArray *SCIMediaImageVersionsFromPhoto(id photo) {
  ...
  NSArray *versions = SCIMediaArrayFromCollection(
      SCIMediaObjectForSelector(photo, @"imageVersions"));
  if (versions.count > 0)
    return versions;

  versions = SCIMediaArrayFromCollection(
      [SCIUtils getIvarForObj:photo name:"_originalImageVersions"]);
  if (versions.count > 0)
    return versions;

  versions = SCIMediaArrayFromCollection(
      SCIMediaObjectForSelector(photo, @"imageVersionDictionaries"));
  if (versions.count > 0)
    return versions;

  versions =
      SCIMediaArrayFromCollection([SCIUtils getIvarForObj:photo
                                                     name:"_imageVersions"]);
  if (versions.count > 0)
    return versions;

  versions = SCIMediaArrayFromCollection(
      [SCIUtils getIvarForObj:photo name:"_imageVersionDictionaries"]);
...
static NSArray *SCIMediaVideoVersionsFromVideo(id video) {
  ...
  NSArray *versions = SCIMediaArrayFromCollection(
      SCIMediaObjectForSelector(video, @"videoVersions"));
  if (versions.count > 0)
    return versions;

  versions = SCIMediaArrayFromCollection(
      SCIMediaObjectForSelector(video, @"videoVersionDictionaries"));
  if (versions.count > 0)
    return versions;

  versions =
      SCIMediaArrayFromCollection([SCIUtils getIvarForObj:video
                                                     name:"_videoVersions"]);
  if (versions.count > 0)
    return versions;

  versions = SCIMediaArrayFromCollection(
      [SCIUtils getIvarForObj:video name:"_videoVersionDictionaries"]);
  return versions.count > 0 ? versions : nil;
}

static NSArray<NSDictionary *> *
SCIMediaSortedVariantsFromVersions(NSArray *versions) {
...
static NSArray<NSDictionary *> *
SCIMediaPhotoVariantDictionaries(id mediaObject) {
  NSMutableArray<NSDictionary *> *variants = [NSMutableArray array];
  ...
  if ([candidates isKindOfClass:[NSArray class]]) {
    [variants
        addObjectsFromArray:SCIMediaSortedVariantsFromVersions(candidates)];
  }

  id photoObject = SCIMediaObjectForSelector(mediaObject, @"photo")
                       ?: SCIMediaObjectForSelector(mediaObject, @"rawPhoto");
  if (photoObject) {
    [variants
        addObjectsFromArray:SCIMediaSortedVariantsFromVersions(
                                SCIMediaImageVersionsFromPhoto(photoObject))];
  }
  return SCIMediaNormalizedAndSortedVariants(variants);
}

static NSArray<NSDictionary *> *
SCIMediaVideoVariantDictionaries(id mediaObject) {
  NSMutableArray<NSDictionary *> *variants = [NSMutableArray array];
  id fieldCacheVariants =
      SCIMediaFieldCacheValue(mediaObject, @"video_versions");
  if ([fieldCacheVariants isKindOfClass:[NSArray class]]) {
    [variants addObjectsFromArray:SCIMediaSortedVariantsFromVersions(
                                      fieldCacheVariants)];
  }

  id videoObject = SCIMediaObjectForSelector(mediaObject, @"video")
                       ?: SCIMediaObjectForSelector(mediaObject, @"rawVideo");
  if (videoObject) {
    [variants
        addObjectsFromArray:SCIMediaSortedVariantsFromVersions(
                                SCIMediaVideoVersionsFromVideo(videoObject))];
  }
  return SCIMediaNormalizedAndSortedVariants(variants);
}
...
static NSString *SCIMediaSubtitle(NSInteger width, NSInteger height,
...
static NSString *SCIMediaPhotoSubtitle(NSInteger width, NSInteger height,
...
static NSArray<SCIMediaOption *> *
SCIMediaBuildPhotoOptions(id mediaObject, NSURL *fallbackURL,
                          NSTimeInterval duration) {
    ...
    option.title =
        (option.width > 0 && option.height > 0)
            ? [NSString stringWithFormat:@"%ld×%ld", (long)option.width,
                                         (long)option.height]
            : (SCIMediaResolutionLabel(option.width, option.height)
                   ?: @"Image");
    option.subtitle = SCIMediaPhotoSubtitle(option.width, option.height,
...
static NSArray<SCIMediaOption *> *
SCIMediaBuildProgressiveVideoOptions(id mediaObject, NSURL *fallbackURL,
                                     NSTimeInterval duration) {
  NSMutableArray<SCIMediaOption *> *options = [NSMutableArray array];
  NSMutableSet<NSString *> *seen = [NSMutableSet set];
  ...
  for (NSDictionary *variant in SCIMediaVideoVariantDictionaries(mediaObject)) {
    ...
    option.duration = duration;
    option.title =
        SCIMediaResolutionLabel(option.width, option.height) ?: @"Video";
    option.subtitle =
        SCIMediaSubtitle(option.width, option.height, option.bandwidth,
...
static NSArray<SCIDashRepresentation *> *
SCIMediaRepresentationsForType(NSArray<SCIDashRepresentation *> *reps,
                               NSString *type) {
  NSPredicate *predicate =
      [NSPredicate predicateWithBlock:^BOOL(
                       SCIDashRepresentation *evaluatedObject,
...
static NSArray<SCIMediaOption *> *
SCIMediaBuildMergedDashOptions(NSArray<SCIDashRepresentation *> *videoReps,
  ...
  for (SCIDashRepresentation *videoRep in videoReps) {
    ...
    option.title = SCIMediaResolutionLabel(videoRep.width, videoRep.height)
                       ?: @"Merged video";
    option.subtitle = SCIMediaSubtitle(
...
static NSArray<SCIMediaOption *> *
SCIMediaBuildVideoOnlyDashOptions(NSArray<SCIDashRepresentation *> *videoReps,
                                  NSTimeInterval duration) {
  NSMutableArray<SCIMediaOption *> *options = [NSMutableArray array];
  for (SCIDashRepresentation *videoRep in videoReps) {
    if (!videoRep.url)
    ...
    option.title = SCIMediaResolutionLabel(videoRep.width, videoRep.height)
                       ?: @"Video only";
    option.subtitle =
        SCIMediaSubtitle(videoRep.width, videoRep.height, videoRep.bandwidth,
...
static NSArray<SCIMediaOption *> *
SCIMediaBuildAudioDashOptions(NSArray<SCIDashRepresentation *> *audioReps,
  ...
  for (SCIDashRepresentation *audioRep in audioReps) {
    ...
    option.subtitle = SCIMediaSubtitle(0, 0, audioRep.bandwidth, duration,
                                       audioRep.codecs, nil);
...
static SCIDashRepresentation *SCIMediaBestMergeAudioRepresentation(
    NSArray<SCIDashRepresentation *> *audioReps) {
  if (audioReps.count == 0)
    return nil;

  NSArray<SCIDashRepresentation *> *sorted =
      [audioReps sortedArrayUsingComparator:^NSComparisonResult(
                     SCIDashRepresentation *lhs, SCIDashRepresentation *rhs) {
...
static SCIMediaOptionSection *
SCIMediaSection(NSString *title, NSArray<SCIMediaOption *> *options) {
...
static SCIMediaAnalysis *SCIMediaAnalyze(id mediaObject, NSURL *photoURL,
                                         NSURL *videoURL,
  ...
  NSArray<SCIMediaOption *> *photoOptions =
      SCIMediaBuildPhotoOptions(mediaObject, photoURL, analysis.duration);
  NSArray<SCIMediaOption *> *progressiveVideoOptions =
      SCIMediaBuildProgressiveVideoOptions(mediaObject, videoURL,
                                           analysis.duration);
  ...
  NSArray<SCIDashRepresentation *> *dashVideo =
      SCIMediaRepresentationsForType(representations, @"video");
  NSArray<SCIDashRepresentation *> *dashAudio =
      SCIMediaRepresentationsForType(representations, @"audio");
  SCIDashRepresentation *bestAudio =
      SCIMediaBestMergeAudioRepresentation(dashAudio);

  NSArray<SCIMediaOption *> *mergedOptions = SCIMediaBuildMergedDashOptions(
      dashVideo, bestAudio, analysis.duration, analysis.ffmpegAvailable);
  NSArray<SCIMediaOption *> *videoOnlyOptions =
      SCIMediaBuildVideoOnlyDashOptions(dashVideo, analysis.duration);
  NSArray<SCIMediaOption *> *audioOptions = SCIMediaBuildAudioDashOptions(
  ...
  if (progressiveVideoOptions.count > 0)
    [sections
        addObject:SCIMediaSection(@"Ready-to-play", progressiveVideoOptions)];
  if (mergedOptions.count > 0)
    [sections addObject:SCIMediaSection(@"Merge Video + Audio (DASH)",
                                        mergedOptions)];
  if (audioOptions.count > 0 && includeAudioOptions)
    [sections addObject:SCIMediaSection(@"Audio Only (DASH)", audioOptions)];
  if (videoOnlyOptions.count > 0)
    [sections
        addObject:SCIMediaSection(@"Video Only (DASH)", videoOnlyOptions)];
  analysis.videoSections = sections;

  return analysis;
}

static SCIMediaOption *SCIMediaTieredOption(NSArray<SCIMediaOption *> *options,
                                            NSString *quality) {
  if (options.count == 0)
    return nil;
...
SCIMediaFirstSelectableOption(NSArray<SCIMediaOption *> *options) {
...
static SCIMediaOption *
SCIMediaFFmpegFreeHighOption(SCIMediaAnalysis *analysis) {
...
SCIMediaResolveDefaultOption(SCIMediaAnalysis *analysis) {
  ...
  if (!analysis.isVideo) {
    ...
    return SCIMediaTieredOption(analysis.photoOptions, quality)
...
- (void)URLSession:(NSURLSession *)session
  ...
  if (!destination && self.completionBlock) {
    self.completionBlock(
        nil,
        [SCIUtils errorWithDescription:@"Failed to move downloaded media"]);
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style
  ...
  [_menuButton setImage:SCIMediaIcon(@"more", kSCIMediaOptionIconPointSize)
  ...
  [NSLayoutConstraint activateConstraints:@[
    ...
    [_previewButton.widthAnchor
        constraintEqualToConstant:kSCIMediaOptionControlSize],
...
@interface SCIMediaTextFieldViewController
...
- (NSArray *)buildSections {
  ...
  if ([SCIUtils getBoolPref:@"downloads_adv_encoding"]) {
    [sections addObject:SCITopicSection(
                            @"Video",
                            @[
                              [SCISetting menuCellWithTitle:@"Video Codec"
                                                   subtitle:nil
                                                       menu:[self codecMenu]],
                              [SCISetting menuCellWithTitle:@"Preset"
                                                   subtitle:nil
                                                       menu:[self presetMenu]],
                              [SCISetting menuCellWithTitle:@"H.264 Profile"
                                                   subtitle:nil
                                                       menu:[self profileMenu]],
                              [SCISetting menuCellWithTitle:@"H.264 Level"
                                                   subtitle:nil
                                                       menu:[self levelMenu]]
                            ],
                            nil)];
    ...
    [sections
        addObject:SCITopicSection(
                      @"Advanced",
                      @[
                        [SCISetting menuCellWithTitle:@"Pixel Format"
                                             subtitle:nil
    ...
    SCISetting *ffmpegInfo = [SCISetting
    ...
    [sections
        addObject:
            SCITopicSection(
                @"Video", @[ [SCISetting menuCellWithTitle:@"Encoding Speed"
                                                  subtitle:nil
                                                      menu:[self speedMenu]] ],
...
- (NSArray *)searchSections {
  ...
  return @[
    ...
    SCITopicSection(
        @"Video",
        @[
          [SCISetting menuCellWithTitle:@"Encoding Speed"
                               subtitle:nil
                                   menu:[self speedMenu]],
          [SCISetting menuCellWithTitle:@"Video Codec"
                               subtitle:nil
...
- (instancetype)initWithAnalysis:(SCIMediaAnalysis *)analysis
                     destination:(SCIDownloadDestination)destination
                selectionHandler:
                    (void (^)(SCIMediaOption *option))selectionHandler {
  self = [super init];
  if (!self)
    return nil;
...
- (void)viewDidLoad {
  ...
  self.tableView.backgroundColor =
      [SCIUtils SCIColor_InstagramGroupedBackground];
  self.tableView.separatorColor = [SCIUtils SCIColor_InstagramSeparator];
  [self.tableView registerClass:_SCIMediaOptionCell.class
  ...
  self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]
      initWithImage:[SCIAssetUtils instagramIconNamed:@"xmark"]
...
- (NSArray<SCIMediaOptionSection *> *)sections {
  ...
  return self.analysis.photoOptions.count > 0
             ? @[ SCIMediaSection(@"Photos", self.analysis.photoOptions) ]
             : @[];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  NSString *previewIconName =
      option.kind == SCIMediaOptionKindPhotoProgressive ? @"photo"
      : option.kind == SCIMediaOptionKindAudioDash      ? @"audio"
                                                        : @"video";
  [cell.previewButton
      setImage:SCIMediaIcon(previewIconName, kSCIMediaOptionIconPointSize)
      forState:UIControlStateNormal];
...
- (UIMenu *)menuForOption:(SCIMediaOption *)option {
  ...
  if (option.primaryURL.absoluteString.length > 0) {
    NSString *title =
        option.kind == SCIMediaOptionKindPhotoProgressive ? @"Copy URL"
        : option.kind == SCIMediaOptionKindAudioDash      ? @"Copy Download URL"
                                                          : @"Copy Video URL";
    [children
        addObject:[UIAction
                      actionWithTitle:title
                                image:SCIMediaIcon(@"link",
    ...
    [children
        addObject:[UIAction
                      actionWithTitle:@"View Image"
                                image:SCIMediaIcon(@"photo",
                                                   kSCIMediaOptionIconPointSize)
                           identifier:nil
                              handler:^(__unused UIAction *action) {
                                [SCIFullScreenMediaPlayer
                                    showRemoteImageURL:option.primaryURL];
                              }]];
  } else {
    NSString *playTitle = option.kind == SCIMediaOptionKindAudioDash
                              ? @"Play Audio"
                              : @"Play Video";
    [children
        addObject:[UIAction
...
static NSString *
SCIMediaCopyLocalFileToPasteboard(NSURL *fileURL, NSError **errorOut,
                                  BOOL showToast,
  ...
  if (!fileURL) {
    ...
    if (showToast && identifier.length > 0) {
      SCINotify(identifier, @"Nothing to copy", nil, @"error_filled",
  ...
  if ([extension isEqualToString:@"m4a"] ||
    ...
    if (data) {
      ...
      if (showToast && identifier.length > 0) {
        SCINotify(identifier, @"Copied audio to clipboard", nil,
                  @"circle_check_filled", SCINotificationToneSuccess);
      }
      return @"Copied audio to clipboard";
    }
  } else if ([SCIDownloadDestinationWriter isVideoFileAtURL:fileURL]) {
    NSData *data = [NSData dataWithContentsOfURL:fileURL];
    if (data) {
      [[UIPasteboard generalPasteboard] setData:data
                              forPasteboardType:@"public.mpeg-4"];
      if (showToast && identifier.length > 0) {
        SCINotify(identifier, @"Copied video to clipboard", nil,
                  @"circle_check_filled", SCINotificationToneSuccess);
      }
    ...
    if (image) {
      ...
      if (showToast && identifier.length > 0) {
        SCINotify(identifier, @"Copied photo to clipboard", nil,
                  @"circle_check_filled", SCINotificationToneSuccess);
      }
  ...
  if (errorOut) {
    *errorOut =
        [SCIUtils errorWithDescription:@"Unable to read the selected file."];
  }
  if (showToast && identifier.length > 0) {
    SCINotify(identifier, @"Copy failed", @"Unable to read the selected file.",
              @"error_filled", SCINotificationToneError);
  }
  return nil;
}

static NSString *SCIMediaSuggestedBasename(id mediaObject,
                                           SCIMediaOption *option) {
  NSString *identifier = nil;
  for (NSString *selectorName in @[ @"pk", @"mediaID", @"id" ]) {
    id value = SCIMediaObjectForSelector(mediaObject, selectorName)
                   ?: SCIMediaKVCObject(mediaObject, selectorName);
  ...
  NSString *suffix = option.kind == SCIMediaOptionKindPhotoProgressive
                         ? @"photo"
                     : option.kind == SCIMediaOptionKindAudioDash ? @"audio"
                                                                  : @"video";
...
static BOOL
SCIMediaShouldSkipDuplicateStart(SCIMediaOption *option,
                                 SCIDownloadDestination destination) {
  ...
  dispatch_once(&onceToken, ^{
    guardQueue = dispatch_queue_create("com.scinsta.media.start-guard",
...
static void SCIMediaPerformOptionDownload(
    SCIMediaOption *option, id mediaObject,
    SCIGallerySaveMetadata *galleryMetadata, SCIDownloadDestination destination,
    BOOL copyToClipboard, NSString *notificationIdentifier, BOOL showProgress,
    UIViewController *presenter, SCIDownloadSourceSurface sourceSurface) {
  if (copyToClipboard)
    destination = SCIDownloadDestinationClipboard;
  if (SCIMediaShouldSkipDuplicateStart(option, destination)) {
    return;
  }

  if (option.kind == SCIMediaOptionKindPhotoProgressive ||
      option.kind == SCIMediaOptionKindVideoProgressive) {
    [SCIDownloadHelpers submitRemoteURL:option.primaryURL
                                    extension:SCIMediaExtensionForOption(option)
                                  destination:destination
      ...
      submitDashDownloadWithPrimaryURL:option.primaryURL
                          secondaryURL:option.secondaryURL
                            optionKind:option.kind
                              basename:SCIMediaSuggestedBasename(mediaObject,
                                                                 option)
...
+ (void)runDashDownloadWithPrimaryURL:(NSURL *)primaryURL
  ...
  if (optionKind == SCIMediaOptionKindAudioDash) {
    audioJob = [[SCIMediaSingleDownloadJob alloc] init];
  ...
  [videoJob startWithURL:primaryURL
      ...
      completion:^(NSURL *videoFileURL, NSError *error) {
        ...
        if (optionKind == SCIMediaOptionKindVideoDashOnly) {
          finishFile(videoFileURL);
          return;
        }
...
+ (BOOL)handleDownloadDestination:(SCIDownloadDestination)destination
  ...
  BOOL includeAudioOptions = (destination == SCIDownloadDestinationShare ||
                              destination == SCIDownloadDestinationGallery ||
                              destination == SCIDownloadDestinationCacheOnly ||
                              destination == SCIDownloadDestinationClipboard);
  SCIMediaAnalysis *analysis = SCIMediaAnalyze(
  ...
  if (resolvedOption) {
    SCIMediaPerformOptionDownload(resolvedOption, mediaObject, galleryMetadata,
  ...
  SCIMediaPresentOptionsSheet(resolvedPresenter, sourceView, analysis,
                              destination, ^(SCIMediaOption *option) {
                                SCIMediaPerformOptionDownload(
...
+ (BOOL)handleCopyActionWithIdentifier:(NSString *)identifier
                       ...
                       galleryMetadata:(SCIGallerySaveMetadata *)galleryMetadata
                         ...
                         sourceSurface:(NSInteger)sourceSurface {
  SCIMediaAnalysis *analysis = SCIMediaAnalyze(
      mediaObject, photoURL, videoURL, SCIDownloadDestinationClipboard, YES);
  if (analysis.photoOptions.count == 0 &&
      analysis.progressiveVideoOptions.count == 0 &&
      analysis.mergedDashOptions.count == 0 &&
      analysis.videoDashOnlyOptions.count == 0 &&
  ...
  if (resolvedOption) {
    SCIMediaPerformOptionDownload(resolvedOption, mediaObject, galleryMetadata,
                                  SCIDownloadDestinationClipboard, YES,
                                  identifier, showProgress, resolvedPresenter,
                                  (SCIDownloadSourceSurface)sourceSurface);
    return YES;
  }

  SCIMediaPresentOptionsSheet(
      resolvedPresenter, sourceView, analysis, SCIDownloadDestinationClipboard,
      ^(SCIMediaOption *option) {
        SCIMediaPerformOptionDownload(
            option, mediaObject, galleryMetadata,
...
+ (NSArray *)encodingSettingsSearchSections {
  SCIMediaEncodingSettingsViewController *controller =
      [[SCIMediaEncodingSettingsViewController alloc] init];
