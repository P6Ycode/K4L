...
@interface SCIDownloadTransfer () <NSURLSessionDownloadDelegate>
@property (nonatomic, strong, nullable) NSURLSession *session;
@property (nonatomic, strong, nullable) NSURLSessionDownloadTask *task;
@property (nonatomic, copy, nullable) SCIDownloadTransferProgressBlock progressBlock;
@property (nonatomic, copy, nullable) SCIDownloadTransferCompletionBlock completionBlock;
...
@implementation SCIDownloadTransfer
...
- (void)downloadURL:(NSURL *)url
      ...
      fileExtension:(NSString *)fileExtension
         stagingDir:(NSString *)stagingDir
           ...
           progress:(SCIDownloadTransferProgressBlock)progress
    ...
    if (!url) {
        completion(nil, SCIDownloadError(SCIDownloadErrorInvalidURL, @"Invalid download URL.", nil));
        return;
    }
    NSString *scheme = url.scheme.lowercaseString;
    if (![scheme isEqualToString:@"http"] && ![scheme isEqualToString:@"https"]) {
        completion(nil, SCIDownloadError(SCIDownloadErrorUnsupportedScheme, @"Only HTTP and HTTPS URLs are supported.", nil));
        return;
    }
...
- (BOOL)validateHTTPResponse:(NSURLResponse *)response error:(NSError **)error {
    ...
    if (status == 403 || status == 404 || status == 410) {
        if (error) *error = SCIDownloadError(SCIDownloadErrorExpiredURL, @"The media URL expired. Refresh and try again.", nil);
        return NO;
    }
    if (error) *error = SCIDownloadError(SCIDownloadErrorHTTPFailure, @"Instagram returned a missing media response.", nil);
    return NO;
}

- (BOOL)validateContentType:(NSString *)mime mediaKind:(SCIDownloadMediaKind)kind {
    if (mime.length == 0 || kind == SCIDownloadMediaKindUnknown) return YES;
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location {
    ...
    if (![self validateContentType:mime mediaKind:self.mediaKind]) {
        [self finishWithPath:nil error:SCIDownloadError(SCIDownloadErrorInvalidContentType, @"Instagram returned an unexpected response.", @"Refresh and try again.")];
        return;
    }
    ...
    if ([attrs[NSFileSize] longLongValue] <= 0) {
        [self finishWithPath:nil error:SCIDownloadError(SCIDownloadErrorEmptyFile, @"The downloaded file was empty.", nil)];
        return;
    }
    ...
    if (![[NSFileManager defaultManager] moveItemAtURL:location toURL:[NSURL fileURLWithPath:dest] error:&moveError]) {
        [self finishWithPath:nil error:SCIDownloadError(SCIDownloadErrorFileMoveFailed, @"Could not store the downloaded file.", moveError.localizedDescription)];
        return;
    }
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    ...
    if (error) {
        if (error.code == NSURLErrorCancelled) {
            [self finishWithPath:nil error:SCIDownloadError(SCIDownloadErrorCancelled, @"Download cancelled.", nil)];
        } else {
...
- (void)finishWithPath:(NSString *)path error:(NSError *)error {
    ...
    self.session = nil;
    self.task = nil;
    SCIDownloadTransferCompletionBlock completion = self.completionBlock;
